...
class CustomClusterManager<T extends ClusterItem> {
  CustomClusterManager(this._items, this.updateMarkers,
      ...
      Future<Marker> Function(Cluster<T>)? stringMarkerBuilder,
      ...
      : markerBuilder = markerBuilder ?? _basicMarkerBuilder,
        stringMarkerBuilder = stringMarkerBuilder ?? _basicMarkerBuilder,
        assert(levels.length <= precision);

  /// Method to build markers
  final Future<Marker> Function(Cluster<T>) markerBuilder;

  final Future<Marker> Function(Cluster<T>) stringMarkerBuilder;
  ...
  void _updateClusters() async {
    ...
    final Set<Marker> stringMarkers = Set.from(await Future.wait(mapMarkers.map((m) => stringMarkerBuilder(m))));
    updateMarkers({...markers, ...stringMarkers});
  }
  ...
  Future<List<Cluster<T>>> getMarkers() async {
    ...
    if (stopClusteringZoom != null && _zoom >= stopClusteringZoom!) return visibleItems.map((i) => Cluster<T>([i])).toList();
